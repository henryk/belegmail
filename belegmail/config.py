...
class ConfigurationManager(object):
    ...
    def load(self, fp):
        ...
        if isinstance(config, dict):
            config = [
                {
                    "name": os.path.basename(getattr(fp, "name", "default")).rsplit(
                        ".", 1
                    )[0],
                    "config": config,
                }
            ...
            raise ConfigParseError(
                "Konfiguration muss entweder ein Dictionary sein, oder eine Liste von Dictionaries."
            )

        for cfg in config:
            if not "name" in cfg or not "config" in cfg:
                raise ConfigParseError(
                    'Konfiguration muss eine Liste von Dictionaries mit den Schlüsseln "name" und "config" sein.'
                )
